...
Sub CreateSignature(SignatureName, templateFileName)
	...
	AddDebug ""
	AddDebug "************************************************************************************************************************************"
	AddDebug "* " & Date & " * Creating Signature " & SignatureName & " using file " & templateFileName
	AddDebug "************************************************************************************************************************************"
	...
	' Get the Standard Users values from Activedirectory
...
IF LEN(templateFilePath) > 0 THEN
	...
	IF LEN(headerHTML) >0 THEN
	 ...
	 TransformCompanyPhone = ReadDefaultSettings("TransformCompanyPhone",headerHTML)
		...
		AddDebug "Set Variable :XmasLastDay 'last work day in December ' -> " & XmasLastDay
		
		' Check xmas details if they are being used in the signature
        if len(xmasfrom) > 0 and len(xmasto) > 0 then 
			
			'AddDebug "Set Variable: xmasfrom 'XMAS' -> "& xmasfrom&"
			...
			if right(ucase(xmasfrom),4) = "YYYY" then
				...
				END IF
				AddDebug "Set Variable :xmasfrom 'Auto Xmas Year' -> " & xmasfrom
				AutoXmas = True
			end if

			IF instr(ucase(xmasfrom),"MM") > 0 then
				xmasfrom = Replace(xmasfrom, "MM", "12")
				AddDebug "Set Variable :xmasfrom 'Auto Xmas Month (December)' -> " & xmasfrom
				AutoXmas = True
			END IF
			...
			IF instr(ucase(xmasfrom),"DD") > 0 then
				...
				AddDebug "Set Variable :xmasfrom 'First Work Day in December' -> " & xmasfrom
				AutoXmas = True
			END IF
			...
			if right(ucase(xmasto),4) = "YYYY" then 
				...
				AddDebug "Set Variable Auto: xmasto 'Year' -> " & xmasto
				AutoXmas = True
			end if
			
			IF instr(ucase(xmasto),"MM") > 0 then
				xmasto = Replace(xmasto, "MM", "01")
				AddDebug "Set Variable Auto: xmasto 'Month (January)' -> " & xmasto
				AutoXmas = True
			END IF
			...
			IF instr(ucase(xmasto),"DD") > 0 then
				...
				IF LEN(NewYearStartWeek) > 0 then 
					AddDebug "Set Variable :Auto xmasto '"& th(int(NewYearStartWeek)) & " Sunday' -> " & xmasto
				ELSE
					AddDebug "Set Variable :Auto xmasto 'Second Sunday' -> " & xmasto
				END IF
				AutoXmas = True
			END IF
			...
			if isBetweenDate(CDate(xmasfrom), CDate(xmasto)) = True or TestXmas = True then 
				...
				if len(xmasimagename) = 0 then 
					...
					AddDebug "Set Variable :xmasimagename 'Was BLANK' -> " &xmasimagename
				else
					imagename = xmasimagename
					AddDebug "Set Variable :imagename 'XmasImageName' -> " &xmasimagename
				end if
			Else
		...
		if MaxSocialIcons then AddDebug "Found " & cstr(MaxSocialIcons) & " " & FoundSocialIcon
		...
		if MaxCombine then AddDebug "Found " & cstr(MaxCombine) & " " & FindCombine
	...
	if len(company) then TemplateTEXT = TemplateTEXT + string(MaxL+1,"-") + VbCrLf+ company + VbCrLf+ string(MaxL+1,"-") + VbCrLf
	...
	if len(address) then TemplateTEXT = TemplateTEXT + address + VbCrLf
	if len(suburb) then TemplateTEXT = TemplateTEXT + suburb + " "  
	if len(city) = 0 then TemplateTEXT = TemplateTEXT + VbCrLf
	if len(city) then TemplateTEXT = TemplateTEXT + city+ " "  
	if len(pobox) then TemplateTEXT = TemplateTEXT + pobox + " "
	if len(state) = 0 then TemplateTEXT = TemplateTEXT + VbCrLf
	if len(state) then TemplateTEXT = TemplateTEXT + state +" "
	if len(postcode) = 0 then TemplateTEXT = TemplateTEXT + VbCrLf
	if len(state) then TemplateTEXT = TemplateTEXT + " " + postcode + VbCrLf
...
Sub UpdateGlobalVarsFromAD() 
    ...
    AddDebug "Geting AD information from LDAP://" & UserDN
...
Function DownloadFile(ByVal filename, ByVal sourceUrl, ByVal destinationDirectory)
    ...
    if httpRequest.status = 200 Then
		...
		AddDebug "Copy File - OK"
    Else
        AddDebug "Copy File - FAILED ("
        DownloadFile = ""
